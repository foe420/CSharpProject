  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-[#111111] p-6 text-white shadow-2xl">
        <h2 className="mb-4 text-2xl font-bold">Upload track/video</h2>
        <form onSubmit={handleUpload} className="space-y-4">
          <div className="rounded-xl border-2 border-dashed border-zinc-700 p-6 text-center transition hover:border-spotify-green">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} className="rounded-full bg-zinc-800 px-4 py-2 text-sm text-white hover:bg-zinc-700">Choose file</button>
          ...
          <div>
            <label className="mb-1 block text-sm text-zinc-400">Genre (optional)</label>
          ...
          <div className="mt-6 flex justify-end gap-3">
            <button type="button" onClick={onClose} className="rounded-full px-4 py-2 text-zinc-400 hover:text-white">Cancel</button>
