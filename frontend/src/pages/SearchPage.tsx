...
export function SearchPage() {
  ...
  return (
    <div className="space-y-6 text-white">
      ...
      <input type="search" placeholder="Nhập tên bài hát, nghệ sĩ..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full rounded-full border border-zinc-700 bg-zinc-900 px-6 py-3 text-white outline-none focus:border-spotify-green" />
      {loading && <p className="text-zinc-400">Đang tìm kiếm...</p>}
      ...
      {!loading && searchTerm && results.length === 0 && (
        <div className="mt-10 text-center text-zinc-500">Không tìm thấy kết quả nào cho "{searchTerm}"</div>
      )}
    </div>
  );
}