...
export function ShareInboxPage() {
  ...
  return (
    <div className="space-y-6 text-white">
      <h1 className="text-3xl font-bold">Share Inbox</h1>
      <div className="flex gap-4 border-b border-zinc-800 pb-2">
        <button onClick={() => setActiveTab('inbox')} className={`pb-2 text-sm font-semibold transition ${activeTab === 'inbox' ? 'border-b-2 border-spotify-green text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Inbox</button>
        <button onClick={() => setActiveTab('sent')} className={`pb-2 text-sm font-semibold transition ${activeTab === 'sent' ? 'border-b-2 border-spotify-green text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Sent</button>
      </div>
      {loading ? (
